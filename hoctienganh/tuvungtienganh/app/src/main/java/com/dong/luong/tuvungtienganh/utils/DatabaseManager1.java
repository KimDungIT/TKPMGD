...
public class DatabaseManager1 {
    ...
    private static final String DATABSE_NAME = "luutru.sqlite";
    private SQLiteDatabase sqLiteDatabase;
    private Context context;

    private static final String TABLE_NAME_VOCABULARY = "anhviet";
    private static final String TABLE_NAME_QUESTION = "tracnghiem";
    private static final String SQL_GET_VOCABULARY = "SELECT * FROM " + TABLE_NAME_VOCABULARY;
    ...
    public ArrayList<SearchModel> getListSearch(String key) {
        openDB();
        Cursor c = sqLiteDatabase.rawQuery("select * From anhviet where tu "+" LIKE '%" +key+"%' limit 0, 50", null);
        if (c == null) {
            return null;
        }
        int indexWord = c.getColumnIndex("tu");
        int indexMean = c.getColumnIndex("nghia");
        String word;
        String mean;
        ...
        while (c.isAfterLast() == false) {
            ...
            word = c.getString(indexWord);
            mean = c.getString(indexMean);
            searchModels.add(new SearchModel(word, mean));
            c.moveToNext();
        }
        c.close();
        closeDB();
    ...
    public ArrayList<GrammarModel> getListGrammar() {
        openDB();
        Cursor c = sqLiteDatabase.rawQuery(SQL_GET_VOCABULARY, null);
        if (c == null) {
            return null;
        }
